...
const Button: React.FC<ButtonProps> = ({ 
  ...
  const variants = {
    ...
    secondary: "bg-slate-700 hover:bg-slate-600 text-slate-200 focus:ring-slate-500 disabled:bg-slate-700/50",
    danger: "bg-red-600 hover:bg-red-500 text-white focus:ring-red-500",
  };

  return (
    <button
      className={`${baseStyles} ${variants[variant]} ${className} ${isLoading || disabled ? 'cursor-not-allowed opacity-70' : ''}`}
